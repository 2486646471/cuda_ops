...
__global__ void mySigmoid(float* d_x, float* d_y, int N) {
...
void cpuSigmoid(float* h_x, float* res, int N){
    for (int i = 0; i < N; i++) {
        res[i] = 1.0f / (1.0f + expf(-h_x[i]));
    }
}
...
int main() {
    ...
    dim3 block(256);
    dim3 grid((n + 255) / 256);
    mySigmoid<<<grid, block>>>(d_x, d_y, n);

    //将结果拷贝回主机
    cudaMemcpy(h_y, d_y, n * sizeof(float), cudaMemcpyDeviceToHost);

    float* res = (float*)malloc(n * sizeof(float));
    cpuSigmoid(h_x, res, n);
    for (int i = 0; i < 10; i++) {
        printf("%f ", res[i]);
    }
    printf("\n");
